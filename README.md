...
| **H3** saturation | vote timing | upvotes plateau early while comments keep accruing | **FALSIFIED** — sublinearity is present from birth (β≈0.66–0.78 at age 2–12h); saturation asymmetry is tiny/fast |
...
- **Baseline reproduced.** Upvotes vs size β ≈ 0.70 (paper's ≈ 0.78) — clearly sublinear; the small
  gap is log-binned OLS here vs. the paper's MLE fit on a slightly different post set.
- **H1.** Net (up − down) β = 0.68 vs upvote β = 0.70; downvote share **1.24%**. Sublinearity is a
  property of *positive* voting, not hidden contention.
- **H2.** Spearman ρ(upvotes, size) = 0.50 vs ρ(upvotes, karma/followers) = 0.13–0.15; the ~0.70
  sublinearity survives within authority terciles.
- **H3.** Both upvotes and comments are ~97% complete within 24h; cross-sectional β is already
  sublinear within hours. The gap is set by *what a post is* early on, not by time dynamics.
- **H4.** Disagreement *and* agreement both rise with size (net consensus flat, ρ = +0.03);
  disagreement's effect on upvotes is positive, not negative; every content stratum stays sublinear.

### Refinements (not new mechanisms)

- **H4b — question / discussion-bait.** The decoupling concentrates in Q&A threads: as comment
  question-rate rises, β(upvotes) collapses **0.74 → 0.32** while β(direct replies) stays ~1, so the
  voting/replying gap widens 0.25 → 0.57. The locus is the *discussion*, not the OP (β identical
  whether or not the OP asks a question). A partial moderator — even zero-question threads are
  sublinear (β ≈ 0.74 < 1).
...
- **Spam filter** (post level): drop posts with <50% unique comment content OR <20% unique author,
  among posts with ≥5 stored comments.
- **β fit**: log-bin by size, average the metric per bin, fit the log-log slope (≥5 points per bin).
